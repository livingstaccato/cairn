...
// sampleListing is what cairndex actually writes, so a recogniser is tested
...
// recogniser that refuses to delete real cairndex output.
...
// cairndex use exactly this filename. An orphan reaching provenOurs is by
// definition unclaimed, and cairndex's own writes are always claimed regardless
// of content, so a real empty search index cairndex wrote never reaches this
...
func TestProvenOursAcceptsCairndexsOwnOutput(t *testing.T) {
	l := sampleListing()
	cases := []struct {
		name string
		body []byte
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			if !provenOurs(writeTemp(t, c.name, c.body)) {
				t.Errorf("cairndex's own %s was not recognised, so a real orphan can never be removed", c.name)
			}
		})
	}
}
...
// unique to cairndex. A generator marker closes that: the shape can be imitated,
...
func TestProvenOursRejectsAListingShapeWithNoGenerator(t *testing.T) {
	...
	if provenOurs(writeTemp(t, "index.json", []byte(foreign))) {
		t.Error("a coincidentally cairndex-shaped listing with no generator was accepted")
	}
}

// The mirror is the deployment cairndex exists for, and in a mirror nearly every
	...
	for _, c := range cases {
		t.Run(c.name+"/"+c.body[:min(12, len(c.body))], func(t *testing.T) {
			if provenOurs(writeTemp(t, c.name, []byte(c.body))) {
				t.Errorf("a foreign %s was accepted as cairndex's, so --remove-orphaned would delete it", c.name)
			}
		})
	}
}
...
// sha256sum -c, so cairndex's is byte-for-byte the shape every other publisher's
// is — a Debian or release mirror ships one that is not cairndex's and not
// distinguishable from cairndex's.
