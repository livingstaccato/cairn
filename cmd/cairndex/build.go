...
func newBuildCmd() *cobra.Command {
	...
	cmd.Flags().BoolVar(&opts.Adopt, "adopt", false,
		"claim output paths that already exist and cairndex does not own, instead of "+
			"refusing them; the way back from a lost .cairndex-manifest.json")
	return cmd
}
...
// cairndex has against overwriting somebody else's files, and waiving it must not
// be something an operator discovers afterwards from a diff.
func reportAdopted(log *slog.Logger, adopted []string, dry bool) {
	if len(adopted) == 0 {
		return
	}
	msg := "claimed output cairndex had no record of writing"
	if dry {
		msg = "would claim output cairndex has no record of writing"
	}
