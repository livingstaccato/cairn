...
func fixture(t *testing.T) (configPath, out string) {
	...
	configPath = filepath.Join(root, "cairndex.yaml")
...
func TestRunBuildMissingConfigFails(t *testing.T) {
	var stderr strings.Builder
	if err := runBuild(context.Background(), filepath.Join(t.TempDir(), "nope.yaml"), "", build.Options{}, &stderr); err == nil {
...
// --version has to name a version an operator can pin `go install` or a
// container tag against, not just prove the flag is wired.
func TestVersionFlagReportsTheVersion(t *testing.T) {
	out, err := exec(t, "--version")
	if err != nil {
		t.Fatalf("--version: %v", err)
	}
	if !strings.Contains(out.String(), version) {
		t.Errorf("expected %q in --version output, got %q", version, out.String())
	}
}
...
func TestChangedToListsOnlyWhatMoved(t *testing.T) {
	...
	var stderr strings.Builder
	if err := runBuild(context.Background(), configPath, list, build.Options{}, &stderr); err != nil {
		t.Fatalf("%v, stderr:\n%s", err, stderr.String())
	}
	first := lines(t, list)
	if len(first) == 0 {
...
func TestBuildRejectsAnUnwritableChangedList(t *testing.T) {
	...
	dir := t.TempDir()
	var stderr strings.Builder
	if err := runBuild(context.Background(), configPath, dir, build.Options{}, &stderr); err == nil {
...
func TestAbsolutePathsAreNotJoinedToTheConfigDirectory(t *testing.T) {
	...
	configPath := filepath.Join(t.TempDir(), "cairndex.yaml")
