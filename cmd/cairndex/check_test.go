...
func checkFixture(t *testing.T) (configPath, root, out string) {
	...
	configPath = filepath.Join(base, "cairndex.yaml")
...
func TestCheckRemovesOrphanedOutputWhenAsked(t *testing.T) {
	...
	// tree.csv is a name cairndex generates but this config does not ask for, so
	// nothing claims it: the exact case Prune cannot reach. It carries cairndex's
	...
	if !strings.Contains(stderr.String(), "removed output cairndex does not own") {
...
func TestCheckKeepsOrphansItCannotProveAreCairndexsOwn(t *testing.T) {
	...
	if readErr != nil {
		t.Fatalf("--remove-orphaned deleted a file nothing showed was cairndex's: %v", readErr)
	}
	if string(got) != string(body) {
		t.Errorf("the file was modified: %q", got)
	}
	if !strings.Contains(stderr.String(), "kept: nothing in its content shows cairndex wrote it") {
