...
export function bindCopy(root, clipboard) {
  for (const button of root.querySelectorAll('[data-cairndex-copy]')) {
    button.addEventListener('click', async (event) => {
      event.preventDefault();
      event.stopPropagation();
      const value = button.dataset.cairndexCopy;
      try {
...
export function init(root) {
  ...
  const toolbar = root.querySelector('[data-cairndex-toolbar]');
...
if (typeof document !== 'undefined') {
  document.addEventListener('DOMContentLoaded', () => {
    for (const root of document.querySelectorAll('.cairndex[data-cairndex-present="styled"]')) {
      init(root);
    }
  });
}
