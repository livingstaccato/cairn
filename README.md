...
> **Status: v0.3.0**, the first release. The engine and the Hugo module are both
> in place and covered end to end. Pin a tag rather than `main`, so the binary
> and the templates that render its output move together. Everything published
> before v0.3.0 is retracted: those tags predate the watcher, `check`, `serve`
> and reproducible output. Pre-1.0, so the config schema and the JSON contract
> may still change; `version: 1` in `cairn.yaml` is checked and a build refuses a
